...
from services.resume_rag_split import split_for_rag
...
class TestMetadata:
    def test_scalar_fields_present(self):
        ...
        assert meta["YoE"] == "5"
        assert meta["highest_education_level"] == "硕士"
        assert meta["location"] == "上海"
...
class TestFullText:
    def test_summary_is_first(self):
        result = split_for_rag(_full_resume())
        assert result["full_text"].startswith("资深后端工程师")
